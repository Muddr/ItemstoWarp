...
public class Commands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command senderCommand, String label, String[] args) {
		...
		plugin.Util.console("Player [" + playerName + "] sent chat command [" + fullCommand + "]", Level.INFO);
			...
			if (player != null) {// Make sure it's a player
				...
				if (args[0].equalsIgnoreCase("list")) {
					if (player.hasPermission("itemstowarp.list") || player.hasPermission("itemstowarp.list.all")) {
						Integer page = 1;
						if (args.length == 1) {
							player.sendMessage(plugin.Util.parseColors("^yellowUse /itw list # for more pages"));
							page = 1;
							plugin.Warps.warpList(playerName, page);
						}
						else {
							...
							if (page > 0) {
								...
								player.sendMessage(plugin.Util.parseColors("^redPage must be a number."));
							}
						}
					}
					else {
				...
				if (args[0].equalsIgnoreCase("mylist")) {
					if (player.hasPermission("itemstowarp.list.own")) {
						Integer page = 1;
						if (args.length == 1) {
							player.sendMessage(plugin.Util.parseColors("^yellowUse /itw mylist # for more pages"));
							page = 1;
							plugin.Warps.warpMyList(playerName, page);
						}
						else {
							page = Integer.parseInt(args[1]);
							if (page > 0) {
						...
						player.sendMessage(plugin.Util.parseColors("^redYou do not have permission to list your warps."));
					}
				}
			}
			else {
